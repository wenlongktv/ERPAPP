...
//点击获得之后
$("aside nav a[href!='#'][href!='/ep']")
.click(function () {
    $this = $(this);
    ahtml = $this.html();
    aparent = $this.parent().parent().parent().find("span").html();
    loadAjaxContent($this.attr("href"), '#content');
    drawBreadCrumb(aparent, ahtml);
    return false;
});

function loadAjaxContent(url, divId)
{
	var div = $(divId);
    if (!div)
    {
